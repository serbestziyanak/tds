...
$( document ).ready( function() {
	...
	/* Soru cevap türüne göre seçenekler gelecek*/
		$( "#soru_cevap_turu_id" ).change( function() {
		$.ajax( {
			 url	: "_modul/ajax/ajax_data.php"
			,type	: "post"
			,data	: {
				 soru_cevap_turu_id		: $( "#soru_cevap_turu_id" ).val()
				,islem					: 'cevap_turune_gore_secenek_ver'
			}
			,async		: true
			,success	: function( sonuc ) {
				$( "#secenekler_label" ).show();
				$( "#soru_secenekler" ).html( sonuc );
			}
			,error		: function() {
				alert( "Seçenekler yüklenemedi" );
			}
		} );
	} );
