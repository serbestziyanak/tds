...
$SQL_ekle			= "INSERT INTO tb_ders_yillari SET ";
$SQL_guncelle 		= "UPDATE tb_ders_yillari SET ";

$alanlar[]		= "universite_id";
$degerler[]		= $_SESSION['universite_id'];

foreach( $_REQUEST as $alan => $deger ) {
	if( $alan == 'islem' or  $alan == 'PHPSESSID' or  $alan == 'dersYili_id') continue;
...
if( $islem == 'guncelle' ) $degerler[] = $dersYili_id;
...
switch( $islem ) {
	...
	case 'sil':
		...
		$tek_fakulte_oku = $vt->select( $SQL_tek_fakulte_oku, array( $dersYili_id ) ) [ 2 ];
		if (count( $tek_fakulte_oku ) > 0) {
			$sonuc = $vt->delete( $SQL_sil, array( $dersYili_id ) );
			if( $sonuc[ 0 ] ) $___islem_sonuc = array( 'hata' => $sonuc[ 0 ], 'mesaj' => 'Kayıt silinrken bir hata oluştu ' . $sonuc[ 1 ] );
		}
	break;
}
$_SESSION[ 'sonuclar' ] 		= $___islem_sonuc;
$_SESSION[ 'sonuclar' ][ 'id' ] = $dersYili_id;
header( "Location:../../index.php?modul=dersYillari&islem=guncelle&dersYili_id=".$dersYili_id );
?>